    ...
    >
      <div className="bg-deepNight border border-amethyst/20 rounded-xl p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-heading text-sm font-medium text-amethyst">
            คำแนะนำสำหรับเจ้า
          </h3>
        </div>
        <ul className="space-y-2">
          {suggestions.map((item, index) => (
            <li
              key={index}
              className="text-sm leading-relaxed text-ghostWhite/80 font-thai pl-3 relative before:content-['·'] before:absolute before:left-0 before:text-amethyst"
            >
              {item}
            </li>
          ))}
