...
export function DashboardNavBar() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-surface/80 backdrop-blur-lg border-t border-surface2/50 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
          ...
          return (
            <Link
              ...
              className={`relative flex flex-col items-center gap-1 py-2.5 px-3 min-w-[64px] transition-colors duration-200 ${
                isActive ? 'text-accentBright' : 'text-inkMuted hover:text-accentFaint'
              }`}
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute -top-px left-3 right-3 h-[2px] rounded-full bg-gradient-to-r from-accentBright to-accentSoft shadow-[0_0_6px_rgba(168,85,247,0.4)]"
